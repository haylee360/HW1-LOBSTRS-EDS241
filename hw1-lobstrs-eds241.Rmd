...
**Assignment submission :** Haylee Oyler With Emma Bea Mitchell, Tom Gibbens-Matsuyama, Ian Morris-Sibaja ------------------------------------------------------------------------
...
# mutate look across the data, for numeric. replace with na_real
# mutate(across(anywhere there is NA replace with NA_real)
...
```{r}
# plot 1: Histogram of counts grouped by reef site
spiny_counts %>% 
    ggplot(aes(x = counts)) +
...
# plot 3: Density of counts grouped by MPA status
spiny_counts %>% 
    ggplot(aes(x = as.factor(year), y = counts)) +
    geom_violin(fill = "green") 

# plot 4: Density of counts grouped by MPA status
...
Our residuals do not fall along the line, therefore they are not normal. This is likely because we have chosen an incorrectly parameterized model for our dataset and system.
...
Dispersion refers to the Overdispersion in the context of this model means that the variance in the lobster counts between the treatment sites is greater than the mean of the lobster counts.

EDIT THIS CHANGE WORDS Dispersion refers to the relationship between the variance of the observed data and the mean. In a perfectly fitting Poisson model, the dispersion is exactly 1, meaning the variance equals the mean. Overdispersion occurs when the variance is greater than the mean. This means our model underestimates the variability of the data due to possible missing variables, dependent and clustering counts, or a an overabundance of zero counts in the data.

**d.** Compare results with previous model, explain change in the significance of the treatment effect

The insignificant p-value for the OLS model tells us that any relationship we observed between the site treatment and the lobster counts could just be due to random chance. However, our poisson model does have a signicant treament effect, indicating it is very unlikely this result is due to chance alone.
...
We can see that our data fits the assumptions of the negative binomial model much better than the poisson model. Our residuals are more uniform, we have less zero-inflation, and our data follows our posterior distribution.

## We can also see that the percent change in lobster observation as a result of the MPA site treatment is also 23%, which is the same as our poisson model.
...
Yes, it seems like our data is robust. When we calculate the percent change in lobster counts, we get approximately 23% across all three models we ran. This tells us that our data is robust and that the treatment effect is stable across different model specifications.

## As a whole, the treatment effect does not seem to be robust.
