...
spiny_counts %>% 
    ...
    stat_summary(fun.y = mean, geom = "point", color = "purple", size = 2)  +
    labs(title = "Lobster Counts by Year",
         x = "Year",
         y = "Lobster Counts")

# plot 4: Lobster size  grouped by site
beeswarm(mean_size ~ site, data = spiny_counts, xlab = "Site", ylab="Lobster size", main="Lobster Size by Site") 
```
...
**Across all three models, we see a percent change of approximately a 23% increase in lobster counts in MPA sites compared to non-MPA sites. It appears our treatment effect is stable. When we calculate the percent change in lobster counts, we get approximately 23% across all three models we ran. This tells us that the treatment effect is stable across different model specifications.**
...
plot_counts %>% ggplot(aes(x = year, y = mean_count, color = mpa, group=mpa)) +
    geom_point() +
    geom_line() +
    labs(title = "Average Lobster Counts by Year and MPA Status",
         x = "Year",
